...
undesirables = groups(groups < avgWidth)


figure('name', "Koncentracija belog po kolonama")
plot(whiteColumns/(max(whiteColumns)))
hold on
plot(0.5*groups/max(groups));
legend("Koncentracija belog", "Grupe");
%%
%{
%Drugi prelaz
groups = groups(groups > avgWidth*0.75)
risingEdges = risingEdges(groups > avgWidth*0.75)
fallingEdges = fallingEdges(groups > avgWidth*0.75)
figure('name', "Drugi prelaz po kolonama")
groups  = fallingEdges - risingEdges
